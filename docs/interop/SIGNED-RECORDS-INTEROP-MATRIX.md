...
**Status:** Informative.
**Authored against:** repository state at the Last checked timestamp.
**Last checked:** 2026-05-27T10:53:25Z.
**Scope note:** this matrix records PEAC's neutral interop mappings against adjacent records, attestations, digests, and payment evidence surfaces. Inclusion is descriptive and does not imply endorsement or dependency by either project.
...
## Tier legend (PEAC coverage)

- **Tier A** — A committed PEAC fixture or conformance vector exists and is fixture-backed. Tier A rows resolve to a path under this repository and are validated by `scripts/verify-interop-matrix.mjs`.
- **Tier B** — A documented mapping or composition recipe exists, but no committed PEAC fixture is included. Tier B rows are omitted from this matrix until fixture-backed.
...
A row may be PEAC Tier A while upstream is still active draft or issue-level. The two dimensions are independent.
...
**Compatibility notes and non-claims.** PEAC records references to ERC-8126-aligned attestation artifacts; it does not standardize ERC-8126 or define risk values under it. ERC-8004 Validation Registry attestation references are observed in the same fixture surface; PEAC does not implement the Validation Registry. ERC-8126 is in Last Call and ERC-8004 is in Draft; the row reflects the state at the Last checked timestamp above and will be re-verified before any wording change is shipped to a public release.
...
**Compatibility notes and non-claims.** PEAC records observations from PEAC-owned x402 surfaces; it does not execute settlement, hold funds, or act as a facilitator. The PEAC record envelope uses `RFC 8785 JCS`; this row does not define canonicalization for x402. The row anchors to the official upstream repository and to PEAC-owned surfaces only. PEAC-side recipes are implementation guidance, not upstream x402 source material.
...
Run `node scripts/verify-interop-matrix.mjs` to check that:

- The matrix file exists and has all three rows above.
- Every Tier A row resolves to its fixture path and PEAC-side surfaces on disk.
- The matrix contains no banned phrasing (relationship overclaims, third-party canonicalization identifiers, vendor-as-default framing).

The verifier exits non-zero on any violation.
