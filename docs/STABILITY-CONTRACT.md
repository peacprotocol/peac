...
| `PEAC_INTERNAL_LEGACY_PATH=1` (env) or `_internal.legacyPath: true` (option) | `@peac/protocol.{issue, verifyLocal, verify}` | Routes the protocol entry points through an alternate validation path. Both flag values currently use the same protocol path; the flag reader is exercised so rollback-path plumbing is validated without changing public behavior. Operator runbook in [`docs/diagnostics/ROLLBACK-PATH.md`](diagnostics/ROLLBACK-PATH.md). |
