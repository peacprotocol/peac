...
| RFC 7493 (I-JSON)                | Standards Track    | Raw I-JSON gate on JWS protected header and payload bytes before JSON parse: rejects duplicate member names, out-of-range numbers, invalid UTF-8, surrogates, and noncharacters | [`packages/crypto/src/ijson.ts`](../packages/crypto/src/ijson.ts), [`sdks/go/ijson.go`](../sdks/go/ijson.go), [`specs/conformance/parity-corpus/ijson-raw-input/`](../specs/conformance/parity-corpus/ijson-raw-input/) |
...
| RateLimit header fields (draft-ietf-httpapi-ratelimit-headers) | Active Internet-Draft | `/v1/verify` emits deployed `RateLimit-Limit` / `RateLimit-Remaining` / `RateLimit-Reset` compatibility fields; the active draft status is tracked separately and may differ from earlier deployed field names. | [`apps/api/src/verify-v1.ts`](../apps/api/src/verify-v1.ts)                               |
...
| RFC 4122 / RFC 9562 (UUIDs)                      | Standards Track                     | `jti` / `rid` generation (UUIDv7)                                                              | [`packages/protocol/src/issue.ts`](../packages/protocol/src/issue.ts)                              |
| RFC 3339 (timestamps)                            | Standards Track / Proposed Standard | `occurred_at` ISO 8601 timestamp formatting in records (`iat` is Unix seconds)                 | wire format                                                                                        |
...
| Standard                                | Status                                                                                   | Role in PEAC                                                                                                                                                                                       | Citation site                                                                             |
| --------------------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| RFC 9700                                | BCP 240, Best Current Practice (published January 2025)                                  | OAuth 2.0 Security Best Current Practice. PEAC may carry digests of OAuth-flow artifacts; PEAC does not implement OAuth 2.0. last_verified 2026-05-09. <https://datatracker.ietf.org/doc/rfc9700/> | [`docs/specs/PROVISIONING-LIFECYCLE-PROFILE.md`](specs/PROVISIONING-LIFECYCLE-PROFILE.md) |
| RFC 6749                                | Standards Track (Proposed Standard)                                                      | OAuth 2.0 Authorization Framework. PEAC may carry references to OAuth flows; PEAC does not implement OAuth 2.0. last_verified 2026-05-09. <https://datatracker.ietf.org/doc/rfc6749/>              | same                                                                                      |
| draft-ietf-oauth-v2-1-15                | Watchlist — Active Internet-Draft (NOT RFC); last updated 2026-03-02; expires 2026-09-03 | OAuth 2.1 is an Internet-Draft as of last_verified 2026-05-09. PEAC does not depend on OAuth 2.1. <https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/>                                        | same                                                                                      |
| draft-ietf-oauth-security-topics-update | Watchlist — Active Internet-Draft (monitor-only)                                         | Tracked for downstream-impact awareness. PEAC does not depend on this draft. last_verified 2026-05-09.                                                                                             | same                                                                                      |
