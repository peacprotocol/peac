...
> Part of [PEAC Adapters](./README.md) | Profile: `peac-x402-offer-receipt/0.2`
...
## What this adapter enforces (and does not)

The x402 Offer/Receipt extension defines a payment handshake. PEAC adds an
...
| Offer-receipt consistency (`resourceUrl`, `network`, time window)                | `verifyConsistency` in `verify.ts`                        |
...
| Max-vs-actual delta audit (`upto` authorized maximum vs actual charged amount at settlement)  | x402 scheme layer and downstream audit tooling |
| Recipient binding as an on-chain check                                                        | Facilitator and chain                          |
| Payer identity verification beyond the signature on the receipt                               | Out of scope for the adapter                   |

The adapter is **scheme-agnostic at the verification layer**: `scheme` is
term-matched as a required string but never interpreted. The `scheme`
identifier is preserved verbatim in the raw artifact at
`proofs.x402.offer`, so downstream auditors can reason about phase semantics
(for example, `upto` authorized maximum vs actual charged) at review time.

See [`docs/specs/X402-PROFILE.md § 3.0`](../specs/X402-PROFILE.md) for the
normative statement and [`x402 Scheme Coverage in PEAC`](../compatibility/x402-scheme-coverage.md)
...
| `receipt_issuedAt_stale`      | 400  | Receipt issuedAt outside validity window         |
