...
PENDING -> ACTIVE -> DEPRECATED -> REVOKED
    ...
    |                     |
    +-----> ACTIVE -------+--- (rotation complete)
```

| State | Can Sign? | Can Verify? | JWKS Presence |
|-------|-----------|-------------|---------------|
| PENDING | No | No | Optional |
| ACTIVE | Yes | Yes | MUST be present |
| DEPRECATED | No | Yes | MUST be present |
| REVOKED | No | No | MUST NOT be present |
...
- Old attestations continue to verify

**Step 3: Remove Old Key (T=rotation_date + grace_period)**
...
X-PEAC-Agent-Identity: eyJ0eXBlIjoicGVhYy9hZ2VudC1pZGVudGl0eSI...
```
...
```json
{
  ...
  "peac": {
    "policy_url": "https://agent.example/.well-known/peac-policy.yaml",
...
The adapter:
1. Extracts `X-PEAC-Agent-Identity` header
2. Verifies identity attestation
3. Returns `x-peac-agent-id` and `x-peac-control-type` to upstream
...
allow {
  identity := input.attributes.request.http.headers["x-peac-agent-identity"]
