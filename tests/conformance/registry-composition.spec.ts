...
describe('registry composition parity', () => {
  it('extension registry has 98 requirements across 14 sections (Sections 21..34 generated by build-extension-registry.mjs)', () => {
    expect(extensionRegistry.total_requirements).toBe(98);
    expect(extensionRegistry.sections).toHaveLength(14);
  });

  it('main registry has 290 total requirements across 32 sections (Section 28 A2A Handoff Records + Section 29 CLI Execution Records + Section 30 Lifecycle Observation Records + Section 31 Provisioning Lifecycle Records + Section 32 Agent Action Records + Section 33 Commerce Mandate Records + Section 34 Gateway Export Records)', () => {
    let total = 0;
    for (const section of registry.sections) {
      total += section.requirements.length;
    }
    expect(total).toBe(290);
    expect(registry.sections).toHaveLength(32);
  });
