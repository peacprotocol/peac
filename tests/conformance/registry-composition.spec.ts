...
describe('registry composition parity', () => {
  it('extension registry has 58 requirements across 10 sections (Sections 21..30 generated by build-extension-registry.mjs)', () => {
    expect(extensionRegistry.total_requirements).toBe(58);
    expect(extensionRegistry.sections).toHaveLength(10);
  });

  it('main registry has 250 total requirements across 28 sections (Section 28 A2A Handoff Records + Section 29 CLI Execution Records + Section 30 Lifecycle Observation Records)', () => {
    let total = 0;
    for (const section of registry.sections) {
      total += section.requirements.length;
    }
    expect(total).toBe(250);
    expect(registry.sections).toHaveLength(28);
  });
