...
describe('registry composition parity', () => {
  it('extension registry has 68 requirements across 11 sections (Sections 21..31 generated by build-extension-registry.mjs)', () => {
    expect(extensionRegistry.total_requirements).toBe(68);
    expect(extensionRegistry.sections).toHaveLength(11);
  });

  it('main registry has 260 total requirements across 29 sections (Section 28 A2A Handoff Records + Section 29 CLI Execution Records + Section 30 Lifecycle Observation Records + Section 31 Provisioning Lifecycle Records)', () => {
    let total = 0;
    for (const section of registry.sections) {
      total += section.requirements.length;
    }
    expect(total).toBe(260);
    expect(registry.sections).toHaveLength(29);
  });
