...
describe('registry composition parity', () => {
  it('extension registry has 88 requirements across 13 sections (Sections 21..33 generated by build-extension-registry.mjs)', () => {
    expect(extensionRegistry.total_requirements).toBe(88);
    expect(extensionRegistry.sections).toHaveLength(13);
  });

  it('main registry has 280 total requirements across 31 sections (Section 28 A2A Handoff Records + Section 29 CLI Execution Records + Section 30 Lifecycle Observation Records + Section 31 Provisioning Lifecycle Records + Section 32 Agent Action Records + Section 33 Commerce Mandate Records)', () => {
    let total = 0;
    for (const section of registry.sections) {
      total += section.requirements.length;
    }
    expect(total).toBe(280);
    expect(registry.sections).toHaveLength(31);
  });
