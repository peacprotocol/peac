...
test('ProblemError - creates RFC 9457 compliant structure', () => {
  const error = new ProblemError(
    400,
    'https://www.peacprotocol.org/problems/test',
    'Test Problem',
  ...
  assert.strictEqual(problemDetails.type, 'https://www.peacprotocol.org/problems/test');
...
test('createProblemDetails - maps error codes correctly', () => {
  ...
  assert.strictEqual(problem.type, 'https://www.peacprotocol.org/problems/invalid-jws-format');
...
test('createProblemDetails - handles unknown error codes', () => {
  ...
  assert.strictEqual(problem.type, 'https://www.peacprotocol.org/problems/unknown-error');
...
test('handleVerifyError - maps JWS format errors', () => {
  ...
  assert.strictEqual(result.body.type, 'https://www.peacprotocol.org/problems/invalid-jws-format');
});
...
test('handleVerifyError - maps unknown key errors', () => {
  ...
  assert.strictEqual(result.status, 422);
  assert.strictEqual(result.body.type, 'https://www.peacprotocol.org/problems/unknown-key-id');
});
...
test('handleVerifyError - handles ProblemError instances', () => {
  const problemError = new ProblemError(
    403,
    'https://www.peacprotocol.org/problems/forbidden',
    'Forbidden',
    'Access denied'
  );
  ...
  assert.strictEqual(result.body.type, 'https://www.peacprotocol.org/problems/forbidden');
