...
| T5  | `receipt_url` fetch amplification | Issuer discovery disabled by default; allowlist-only JWKS             | Untrusted issuer test confirms no fetch                                          | No fetch for non-allowlisted issuers                    |
