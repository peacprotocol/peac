...
Runnable examples that demonstrate PEAC Protocol integration patterns. Examples expose either `pnpm demo` or profile-specific commands such as `pnpm issue` and `pnpm verify`.
...
| [`mcp-gateway-receipts`](./mcp-gateway-receipts/)                               | Gateway-mediated MCP tool calls: policy decisions (allow and deny), content digests, redaction facts, signed tool-definition manifest.                                        |
| [`gateway-decision-evidence`](./gateway-decision-evidence/)                     | Terminal gateway access decisions, mandatory non-issuance for intermediate/check/handling states, and profile-aware offline verification under an explicit issuer/key policy. |
...
| [`agent-action-records`](./agent-action-records/)                               | Agent action records (generic, all 6 `*-observed` event kinds: invoked / delegated / approved / denied / cancelled / timed-out).                                              |
| [`dotnet-quickstart`](./dotnet-quickstart/)                                     | .NET 10 example that verifies committed Wire 0.2 agent-action records offline with one direct NSec.Cryptography PackageReference (not a PEAC .NET SDK).                       |
| [`commerce-mandate-records`](./commerce-mandate-records/)                       | Commerce mandate records (generic, all 7 `*-observed` event kinds: mandate / authorization / capture / void / refund / settlement / budget).                                  |
| [`gateway-export-records`](./gateway-export-records/)                           | Gateway export records (generic, all 8 `*-observed` event kinds: 7 settlement/recovery states + 1 facilitator-timeout trigger observation).                                   |
...
| [`counterparty-acknowledgment-records`](./counterparty-acknowledgment-records/) | Two independently signed single-issuer records where a provider record acknowledges a payer payment record by `(iss, jti, receipt_ref)`, verified offline.                    |
| [`action-approval-records`](./action-approval-records/)                         | Single-issuer agent-action records for the action-approval flow (approve or deny an action intent; report the invocation on the approved path), verified offline.             |
| [`agent-run-lineage-records`](./agent-run-lineage-records/)                     | An agent runtime exports reported run events plus a run-finalization record with a Merkle commitment over the covered records, verified offline.                              |
