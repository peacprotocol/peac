#!/usr/bin/env node
/**
 * Issue signed PEAC records for each gateway-export fixture.
 *
 * Reads every JSON file under ./fixtures/, validates the extension
 * payload through validateGatewayExport, signs an interaction record
...
const TYPE_BY_EVENT_KIND = Object.fromEntries(
  GATEWAY_EXPORT_TYPE_URIS.map((u) => [u.replace('org.peacprotocol/', ''), u])
);

const ISSUER = 'https://gateway.example.com';
const KID = 'gateway-export-records-demo';

async function main() {
  console.log('=== Gateway export records: issue ===\n');
  ...
  for (const file of fixtureFiles) {
    ...
    const validation = validateGatewayExport(fixture);
    if (!validation.ok) {
    ...
    const result = await issue({
      ...
      kid: KID,
      extensions: {
        [GATEWAY_EXPORT_EXTENSION_KEY]: fixture,
      },
    });
