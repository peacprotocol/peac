/**
 * PEAC Shared Worker Core - Verification Logic
 *
 ...
 * - Replay protection is required unless unsafeAllowNoReplay=true
 *
...
export async function handleVerification(
  request: RequestLike,
  config: WorkerConfig,
  options: Omit<VerifyTapOptions, 'warnNoReplayStore'>
  ...
  // Non-TAP requests return 402 challenge
  if (!result.isTap) {
    const problem = createProblemDetails(
    ...
    return {
      action: 'challenge',
      status: 402,
      errorCode: ErrorCodes.RECEIPT_MISSING,
      problem,
      requestUrl: request.url,
    };
  }
