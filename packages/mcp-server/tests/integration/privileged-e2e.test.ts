...
describe.skipIf(!CLI_EXISTS && !IS_CI)('integration/privileged-e2e', () => {
  ...
  it('issue then verify round-trip via stdio: Wire 0.2 issue and verify', async () => {
    // MCP peac_issue now produces Wire 0.2 JWS (PR #472).
    // verifyLocal() is Wire 0.2 only; the round-trip should succeed.
    const client = createStdioClient([
      '--issuer-key',
      `file:${keyPath}`,
      '--issuer-id',
    ...
    try {
      ...
      // Call peac_issue (produces Wire 0.2 JWS)
      client.send({
        jsonrpc: '2.0',
        id: 2,
        method: 'tools/call',
      ...
      // Call peac_verify with the issued Wire 0.2 JWS
      client.send({
        jsonrpc: '2.0',
        id: 3,
        method: 'tools/call',
      ...
      // Wire 0.2 JWS verified successfully
