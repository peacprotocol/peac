...
function ensureCapabilitiesMemoized(): { capabilities: PEACCapabilities; etag: string; json: string } {
...
export async function handleCapabilities(
  ...
  try {
    ...
    if (
      clientETag === capabilitiesETag ||
      clientLastModified === CAPABILITIES_LAST_MODIFIED
    ) {
      timer({ status: 304 });
      logger.debug(
    ...
    logger.info(
      { version: CAPABILITIES_VERSION },
      "Capabilities served successfully"
    );
  } catch (error) {
    timer({ status: 500 });
