...
describe('Version Negotiation (strict 0.9.10 only; no legacy)', () => {
  ...
  it('echoes 0.9.10 when no header present', async () => {
    const res = await request(app).get('/test');
    expect(res.status).toBe(200);
    expect(res.headers['x-peac-protocol-version']).toBe('0.9.10');
  ...
  it('accepts exact current version via x-peac-protocol-version', async () => {
    const res = await request(app).get('/test').set('x-peac-protocol-version', '0.9.10');
    expect(res.status).toBe(200);
    expect(res.headers['x-peac-protocol-version']).toBe('0.9.10');
  });
  ...
  it('rejects 0.9.4 via x-peac-protocol-version with 426 + supported list', async () => {
    ...
    expect(res.headers['x-peac-protocol-version-supported']).toBe('0.9.10');
  });
  ...
  it('rejects legacy header name x-peac-version (any value) with 426', async () => {
    const res = await request(app).get('/test').set('x-peac-version', '0.9.10');
    expect(res.status).toBe(426);
    expect(res.headers['x-peac-protocol-version-supported']).toBe('0.9.10');
  });
  ...
  it('rejects when both x-peac-protocol-version and x-peac-version are present', async () => {
    const res = await request(app)
      .get('/test')
      .set('x-peac-protocol-version', '0.9.10')
      .set('x-peac-version', '0.9.10');
    expect(res.status).toBe(426);
    expect(res.headers['x-peac-protocol-version-supported']).toBe('0.9.10');
  });
