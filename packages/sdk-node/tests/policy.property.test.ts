...
describe('Policy Property-Based Tests', () => {
    ...
    it('should handle valid attribution patterns', () => {
      fc.assert(
        fc.property(
          ...
          ({ name, url, version }) => {
            const attribution = version ? `${name} (${url}) [${version}]` : `${name} (${url})`;
  ...
  describe('Policy validation bounds', () => {
    it('should validate retention_days bounds', () => {
      fc.assert(
        fc.property(fc.integer({ min: -100, max: 500 }), (retentionDays) => {
          const policy = {
            version: '0.9.11',
            site: {
              name: 'Test Site',
          ...
          if (isValidRange) {
            ...
            return expect(
              fetchPolicy('https://test.example.com/.well-known/peac'),
            ).resolves.toMatchObject({ privacy: { retention_days: retentionDays } });
          } else {
            // Should throw for invalid range
            return expect(fetchPolicy('https://test.example.com/.well-known/peac')).rejects.toThrow(
    ...
    it('should validate max_rows bounds', () => {
      fc.assert(
        fc.property(fc.integer({ min: -1000, max: 2000000 }), (maxRows) => {
          const policy = {
            version: '0.9.11',
            site: {
              name: 'Test Site',
  ...
  describe('Cache behavior properties', () => {
    it('should respect TTL bounds', () => {
      fc.assert(
        fc.property(fc.integer({ min: 0, max: 3600 }), async (ttlSec) => {
          const policy = {
            version: '0.9.11',
            site: {
              name: 'Test Site',
