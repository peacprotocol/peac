...
export function assertProtocolVersions(
  ...
  if (!isCompatibleProtocol(pv)) {
    ...
    throw createProblem(409, 'version-conflict', 
  ...
  if (wv !== expectedWireVersion) {
    throw createProblem(409, 'wire-version-mismatch',
  ...
  if (pv !== VERSION_CONFIG.CURRENT_PROTOCOL) {
    console.warn(`Accepted compatible protocol_version=${pv} (current: ${VERSION_CONFIG.CURRENT_PROTOCOL})`);
  }
}

export function assertCrawlerType(ct: unknown): void {
  if (typeof ct !== 'string' || ct.length === 0) {
    throw createProblem(400, 'missing-crawler-type', 
      'crawler_type is required and must be a non-empty string'
    );
  }
  
  if (!isValidCrawlerType(ct)) {
    throw createProblem(400, 'invalid-crawler-type', 
...
export function assertTimestamp(
  ...
  if (tsTime > now + maxSkew) {
    throw createProblem(400, 'timestamp-future', `${fieldName} cannot be more than 30s in the future`);
  }
...
export function validateNonce(kid: string, nonce: string, nonceStore: Set<string>): void {
  ...
  if (nonceStore.has(nonceKey)) {
    throw createProblem(409, 'nonce-replay', 'Nonce has already been used (replay attack detected)');
  }
...
function getExpectedWireVersion(kind: 'receipt' | 'purge' | 'discovery'): string {
  switch (kind) {
    case 'receipt': return VERSION_CONFIG.REQUIRED_WIRE_RECEIPT;
    case 'purge': return VERSION_CONFIG.REQUIRED_WIRE_PURGE;
    case 'discovery': return VERSION_CONFIG.REQUIRED_WIRE_DISCOVERY;
    default: throw new Error(`Unknown document kind: ${kind}`);
  }
}
