...
export async function verifyPurgeReceipt(jws: string, keys: KeySet): Promise<{ hdr: any; purge: PurgeReceipt }> {
...
async function verifyDocument(
  ...
  // Validate media type
  const expectedMediaType = expectedType === 'receipt' 
    ? 'application/peac-receipt+jws'
    : 'application/peac-purge+jws';
...
function enforceReceiptInvariants(receipt: Receipt): void {
  ...
  if (receipt.enforcement?.method === 'http-402') {
    if (!receipt.payment) {
      throw new Error('Receipt invariant violation: payment required when enforcement.method="http-402"');
    }
    
    if (!receipt.payment.rail || !receipt.payment.amount || !receipt.payment.currency) {
      throw new Error('Receipt invariant violation: payment must include rail, amount, and currency');
    }
  }
