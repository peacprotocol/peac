...
export async function verifyRDNS(
  ...
  try {
    ...
    // 3) Suffix validation
    const allowedSuffixes = discovery.crawler_verification?.known_crawlers
      ?.flatMap(crawler => crawler.rdns_suffixes || []) || [];
    
    const suffix_ok = allowedSuffixes.length === 0 || 
...
export function verifyIPRange(reqIp: string, discovery: Discovery): boolean {
  const knownRanges = discovery.crawler_verification?.known_crawlers
    ?.flatMap(crawler => crawler.ip_ranges || []) || [];
...
export function verifyUserAgent(userAgent: string, discovery: Discovery): boolean {
  ...
  const knownPatterns = discovery.crawler_verification?.known_crawlers
    ?.flatMap(crawler => crawler.user_agents || []) || [];
