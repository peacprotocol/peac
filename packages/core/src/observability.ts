...
class MetricsCollector {
  ...
  getCurrentMetrics(): Partial<PerfMetrics> {
    return {
      ...
      memory_usage_mb: process.memoryUsage ? Math.round(process.memoryUsage().rss / 1024 / 1024) : 0,
...
export class HealthChecker {
  ...
  async checkHealth(): Promise<HealthStatus> {
    ...
    // Overall status
    const status = this.determineOverallStatus(meets_slo, redis_status, external_apis, security_status);
  ...
  private async getSecurityStatus(): Promise<{
    ...
    const violations = recent_events.filter(e => 
      e.type === 'replay_detected' || e.type === 'nonce_reused'
    ).length;
...
export const observabilityHandlers = {
  ...
  health: async () => {
    ...
    const status_code = health.status === 'healthy' ? 200 : health.status === 'degraded' ? 206 : 503;
