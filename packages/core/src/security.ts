...
export class ReplayDetector {
  ...
  async checkReplay(receipt: Receipt): Promise<{ valid: boolean; reason?: string }> {
    ...
    if (this.config.check_timestamp) {
      ...
      if (issued_at > now + 60000) { // 60 second tolerance
  ...
  async rotateKeys(): Promise<{ created: string[]; expired: string[] }> {
    ...
    for (const key of old_keys) {
      if (this.config.overlap_period_days > 0) {
        const expires_at = new Date(now.getTime() + (this.config.overlap_period_days * 24 * 60 * 60 * 1000));
        key.status = 'rotating';
...
export class SecurityRateLimit {
  ...
  constructor(private max_attempts = 5, private window_ms = 60000) {}
...
export class SecurityAuditor {
  ...
  getSecurityReport(): {
    ...
    const recent_critical = this.events
...
export async function validateReceiptSecurity(
  ...
  if (issued_at > now + 60000) { // 60 second clock skew
