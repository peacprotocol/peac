...
export class CrawlerControlRegistry {
  ...
  async verify(
    ...
    const aggregated: VerificationResult = {
      provider: 'aggregator',
      result: aggregation.decision === 'allow' ? 'trusted' : 
              aggregation.decision === 'block' ? 'unverified' : 'suspicious',
  ...
  private aggregate(results: VerificationResult[]): AggregationResult {
    ...
    switch (strategy) {
      ...
      case 'weighted':
      default: {
        const totalWeight = validResults.reduce((sum, r) => sum + this.getEffectiveWeight(r.provider), 0);
        if (totalWeight === 0) {
          trust_score = 0.3;
        } else {
          trust_score = validResults.reduce((sum, r) => {
            const weight = this.getEffectiveWeight(r.provider);
            const score = r.result === 'trusted' ? r.confidence : 
                         r.result === 'suspicious' ? r.confidence * 0.5 : 0.1;
