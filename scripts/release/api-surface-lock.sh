...
extract_exports() {
  local pkg_name="$1"
  # Find the package's dist/index.d.ts and extract exported names
  local pkg_dir
  pkg_dir=$(node -e "
    const path = require('path');
    const resolve = require.resolve('$pkg_name/package.json', { paths: ['$REPO_ROOT'] });
    process.stdout.write(path.dirname(resolve));
  " 2>/dev/null || echo "")
  ...
  types_file=$(node -e "
    ...
    const types = pkg.types || pkg.typings || (pkg.exports && pkg.exports['.'] && (pkg.exports['.'].types || (pkg.exports['.'].import && pkg.exports['.'].import.types)));
    if (types) process.stdout.write(require('path').resolve('$pkg_dir', types));
  " 2>/dev/null || echo "")
