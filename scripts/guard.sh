...
# wire/0.9/ -> wire/0.1/
# Allow in: CHANGELOG, guard script, migration docs, versioning doctrine (historical), deprecated packages, tests pending migration, Go SDK, security / stability / threat docs that must name the legacy identifier to describe its deprecated state, v0.13.0 package-status doc (archive-scheduled rows must name the legacy wire they retire), archive/ (excluded by path filter below).
