...
NPM_ALLOW='^(IMPLEMENTATION_STATUS\.md|README\.md|packages/.*/README\.md|(docs/)?RELEASING\.md|CHANGELOG\.md|docs/ROADMAP\.md|docs/maintainers/(RELEASING|NPM_PUBLISH|RELEASE-INTEGRITY|SECURITY-POSTURE).*\.md|docs/(VERIFY-RELEASE|guides/edge/|release/)|scripts/(guard\.sh|pack-smoke\.mjs|pack-.*\.sh|otel-smoke\.sh|check-readme-consistency\.sh|publish-manifest\.json|setup-trusted-publishing\.sh|validate-mcp-install\.mjs|release/pack-install-smoke(-registry)?\.(sh|mjs)|verify-dist-private-leaks\.mjs)|packages/net/node/scripts/test-pack-install\.mjs|packages/capture/core/scripts/test-exports\.mjs|\.github/workflows/(publish|promote-latest|publish-mcp-registry)\.yml|integrator-kits/|surfaces/distribution/|llms\.txt|examples/hello-world/|tests/tooling/disc-mention-scope\.test\.ts|docs/MIGRATION_CURRENT\.md|archive/discovery/README\.md)'
...
# wire/0.9/ -> wire/0.1/
# Allow in: CHANGELOG, guard script, migration docs, versioning doctrine (historical), deprecated packages, tests pending migration, Go SDK, security / stability / threat docs that must name the legacy identifier to describe its deprecated state, v0.13.0 package-status doc (archive-scheduled rows must name the legacy wire they retire), machine-readable surface status that records the archive note, the post-release npm deprecate script (must name the legacy receipt format in its deprecation message), workspace-private @peac/compat migration-class taxonomy that classifies the legacy boundary as `impossible` (must name the legacy identifier to define the verdict), archive/ (excluded by path filter below).
LEGACY_WIRE_ALLOW='^(CHANGELOG\.md|SECURITY\.md|REPO_SURFACE_STATUS\.json|scripts/(guard\.sh|lint-schemas\.mjs|verify-trust-artifacts\.mjs|release/npm-deprecate-v0\.13\.0\.sh)|docs/(migration/|MIGRATION_CURRENT\.md|PACKAGE_STATUS\.md|PACKAGE_STATUS_V0\.13\.0_PARITY\.md|specs/VERSIONING\.md|STABILITY-CONTRACT\.md|THREAT_MODEL\.md|baselines/BASELINE-v0\.13\.0\.md|release-notes/v0\.13\.0\.md)|packages/(core|sdk-js)/|packages/(crypto|protocol|schema)/(tests|openapi|src/(index|types|envelope))|packages/compat/(src/taxonomy\.ts|__tests__/taxonomy\.test\.ts|spec/MIGRATION-CLASSES\.md):|tests/(golden|vectors)/|specs/(kernel/README|conformance/fixtures/bundle/invalid)|sdks/go/|examples/x402-node-server/)'
