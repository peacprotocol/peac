...
LEGACY_WIRE_ALLOW='^(CHANGELOG\.md|SECURITY\.md|REPO_SURFACE_STATUS\.json|scripts/(guard\.sh|lint-schemas\.mjs|verify-trust-artifacts\.mjs|release/npm-deprecate-v0\.13\.0\.sh)|docs/(migration/|MIGRATION_CURRENT\.md|PACKAGE_STATUS\.md|PACKAGE_STATUS_V0\.13\.0_PARITY\.md|specs/VERSIONING\.md|STABILITY-CONTRACT\.md|THREAT_MODEL\.md|reboot/BASELINE-v0\.13\.0\.md)|packages/(core|sdk-js)/|packages/(crypto|protocol|schema)/(tests|openapi|src/(index|types|envelope))|tests/(golden|vectors)/|specs/(kernel/README|conformance/fixtures/bundle/invalid)|sdks/go/|examples/x402-node-server/)'
