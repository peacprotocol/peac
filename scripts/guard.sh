...
NPM_ALLOW='^(IMPLEMENTATION_STATUS\.md|README\.md|RELEASING\.md|CHANGELOG\.md|docs/maintainers/(RELEASING|NPM_PUBLISH).*\.md|docs/guides/edge/|scripts/pack-.*\.sh)'
...
# wire/0.9/ -> wire/0.1/
# Allow in: CHANGELOG, guard script, migration docs, deprecated packages, tests pending migration, Go SDK
LEGACY_WIRE_ALLOW='^(CHANGELOG\.md|scripts/(guard\.sh|lint-schemas\.mjs)|docs/migration/|packages/(core|sdk-js)/|packages/(crypto|protocol|schema)/(tests|openapi|src/(index|types|envelope))|tests/(golden|vectors)/|specs/(kernel/README|conformance/fixtures/bundle/invalid)|sdks/go/|examples/x402-node-server/)'
