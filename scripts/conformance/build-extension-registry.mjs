 ...
 *      - Owns non-WIRE02 requirements (98 IDs across 14 sections)
 ...
 *        A2A-HANDOFF, CLI-EXEC, LIFE-OBS, PROV-LIFE, AGENT-ACT, COMM-MAN,
 *        GATE-EXP
 *
...
const EXTENSION_REQUIREMENTS = [
  ...
  // --- Section 33: Commerce Mandate Records ---
  {
    section: 33,
    title: 'Commerce Mandate Records',
    anchor: 'commerce-mandate-records',
    governing_spec: 'docs/specs/COMMERCE-MANDATE-RECORDS.md',
    introduced_in: '0.14.3',
    last_reviewed_in: '0.14.3',
    requirements: [
      {
        ...
        keyword: 'MUST',
        summary:
          'Validator rejects 20 forbidden top-level payment-data keys with commerce.mandate.inline_payment_data_blocked',
        source_fragment:
          'commerce.mandate.inline_payment_data_blocked rejects 20 forbidden top-level payment-data keys (card_number / pan / cvv / cvc / expiry_date / card_holder_name / billing_address / shipping_address / token / raw_token / bearer_token / api_key / secret / private_key / private_key_pem / credential / password / connection_string / iban / bank_account) at extension top level',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'COMM-MAN-002',
        keyword: 'MUST',
        summary:
          'All *_ref fields validated by the OpaqueRefSchema grammar (no whitespace, no @, recognized prefix, byte-bounded); violations surface commerce.mandate.opaque_ref_grammar_violation; non-string values surface commerce.mandate.ref_must_be_string',
        source_fragment:
          'all *_ref fields validated by the OpaqueRefSchema grammar (mandate_ref / merchant_ref / payer_ref / authorization_ref / capture_ref / void_ref / refund_ref / settlement_ref / budget_ref / caller_ref / policy_ref / upstream_artifact_ref / parent_ref / scheme_ref)',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'COMM-MAN-003',
        keyword: 'MUST',
        summary:
          'All amount fields use AmountMinorStringSchema; numeric, decimal, empty, and unsafe forms reject with commerce.mandate.invalid_amount_minor',
        source_fragment:
          'AmountMinorStringSchema bounded decimal-integer string for amount_minor and max_amount_minor; numeric, decimal, empty values reject with commerce.mandate.invalid_amount_minor',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'COMM-MAN-004',
        keyword: 'MUST',
        summary:
          'Per-event-kind required fields enforced; missing mandate_ref, observed_at, and event-kind-specific required fields surface commerce.mandate.missing_required_field',
        source_fragment:
          'per-event-kind required fields enforced via discriminated union; mandate_ref and observed_at are common required; per-kind: mandate (merchant_ref, payer_ref); authorization (authorization_ref, amount_minor, currency); capture (authorization_ref, capture_ref, amount_minor, currency); void (authorization_ref, void_ref); refund (refund_ref, amount_minor, currency); settlement (settlement_ref, amount_minor, currency, settlement_state); budget (budget_ref)',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'COMM-MAN-005',
        keyword: 'MUST',
        summary: 'Unknown event_kind rejects with commerce.mandate.invalid_event_kind',
        source_fragment:
          'unknown event_kind rejects with commerce.mandate.invalid_event_kind; closed-enum discriminator over 7 *-observed event kinds (mandate / authorization / capture / void / refund / settlement / budget)',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'COMM-MAN-006',
        keyword: 'MUST',
        summary:
          'Finality-synthesis boundary: settlement_state on any non-settlement event kind rejects with commerce.mandate.finality_synthesis_blocked',
        source_fragment:
          'settlement_state on commerce-authorization-observed (or any non-settlement variant) rejects with commerce.mandate.finality_synthesis_blocked; settlement finality may only be reported via commerce-settlement-observed',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'COMM-MAN-007',
        keyword: 'MUST',
        summary:
          'scheme_id (bounded grammar) and scheme_ref (opaque) are mutually exclusive; both present rejects with commerce.mandate.scheme_conflict',
        source_fragment:
          'scheme_id and scheme_ref are mutually exclusive; setting both rejects with commerce.mandate.scheme_conflict; scheme_id must match bounded grammar [a-z0-9._:/+-]{1,128} (lowercase preferred; no whitespace, no @)',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'COMM-MAN-008',
        keyword: 'MUST',
        summary:
          'Commerce mandate records round-trip through @peac/protocol.issue() and verifyLocal()',
        source_fragment:
          'commerce mandate records round-trip through @peac/protocol.issue() and verifyLocal()',
        enforcement_class: 'hard_fail',
      },
      {
        ...
        keyword: 'MUST',
        summary:
          'Every type URI in COMMERCE_MANDATE_TYPE_URIS maps to org.peacprotocol/commerce-mandate in TYPE_TO_EXTENSION_MAP; validateCommerceMandateForType rejects unknown type URIs (type_uri_unknown) and type URI / event_kind mismatches (type_event_kind_mismatch)',
        source_fragment:
          'every URI in COMMERCE_MANDATE_TYPE_URIS maps to org.peacprotocol/commerce-mandate in TYPE_TO_EXTENSION_MAP; validateCommerceMandateForType runtime guard: unknown type URI -> commerce.mandate.type_uri_unknown; mismatched event_kind -> commerce.mandate.type_event_kind_mismatch',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'COMM-MAN-010',
        keyword: 'MUST',
        summary:
          'Spec boundary text is normative and vendor-neutral; PEAC does not authorize payments, process payments, settle funds, enforce mandates, compute payment finality, evaluate budgets, validate payment rails, or vouch for the legal validity of any commerce decision',
        source_fragment:
          'PEAC does not authorize payments, process payments, settle funds, enforce mandates, compute payment finality, evaluate budgets, validate payment rails, or vouch for the legal validity of any commerce decision; commerce decisions are reported by the caller',
        enforcement_class: 'hard_fail',
      },
    ],
  },

  // --- Section 34: Gateway Export Records ---
  {
    section: 34,
    title: 'Gateway Export Records',
    anchor: 'gateway-export-records',
    governing_spec: 'docs/specs/GATEWAY-EXPORT-RECORDS.md',
    introduced_in: '0.14.3',
    last_reviewed_in: '0.14.3',
    requirements: [
      {
        ...
        keyword: 'MUST',
        summary:
          'Validator rejects 19 forbidden top-level payment-data keys with gateway.export.inline_payment_data_blocked. value_minor is intentionally NOT in this list (rejected as gateway.export.unknown_field via the strict variant schema; the single canonical money field is amount_minor)',
        source_fragment:
          'gateway.export.inline_payment_data_blocked rejects 19 forbidden top-level payment-data keys (transaction_data / raw_tx / tx_hash_value / nonce / raw_nonce / payer / pay_to / payTo / payment_payload / authorization / authorization_payload / card_number / pan / cvv / token / bearer_token / api_key / private_key / credential) at extension top level. value_minor is NOT in this list; it is rejected as gateway.export.unknown_field via the strict variant schema (single canonical money field is amount_minor)',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-002',
        keyword: 'MUST',
        summary:
          'All *_ref fields validated by the OpaqueRefSchema grammar (no whitespace, no @, recognized prefix, byte-bounded); violations surface gateway.export.opaque_ref_grammar_violation; non-string values surface gateway.export.ref_must_be_string',
        source_fragment:
          'all *_ref fields validated by the OpaqueRefSchema grammar (gateway_ref / payment_ref / facilitator_ref / tx_ref / nonce_ref / upstream_artifact_ref / caller_ref / parent_ref / payer_ref / pay_to_ref / settlement_ref / failure_reason_ref / chain_ref)',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-003',
        keyword: 'MUST',
        summary:
          'amount_minor uses the shared AmountMinorStringSchema grammar plus a Gateway Export non-negative profile constraint; numeric, decimal, comma-formatted, empty, AND negative values reject with gateway.export.invalid_amount_minor; the base amount_minor field is the single canonical money field (EIP-3009 value maps here when four-tuple refs are present); no separate value_minor field is defined (records carrying value_minor reject as gateway.export.unknown_field via the strict variant schema, not as inline_payment_data_blocked)',
        source_fragment:
          'amount_minor uses the shared AmountMinorStringSchema grammar (bounded base-10 integer string) wrapped in a Gateway Export non-negative profile constraint (negative values reject); numeric, decimal, comma-formatted, empty, and negative values reject with gateway.export.invalid_amount_minor; single-canonical-money-field invariant: amount_minor is the only monetary field per record; records carrying value_minor reject as gateway.export.unknown_field via the strict variant schema',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-004',
        keyword: 'MUST',
        summary:
          'Per-event-kind required fields enforced; missing gateway_ref, payment_ref, observed_at, and event-kind-specific required fields surface gateway.export.missing_required_field',
        source_fragment:
          'per-event-kind required fields enforced via discriminated union; gateway_ref / payment_ref / observed_at are common required; per-kind: payment-submitted (submitted_at); facilitator-timeout (timeout_at, timeout_profile); settlement-unresolved (last_checked_at, check_count); settlement-polling (poll_count, polling_strategy); settlement-confirmed (confirmed_at, settlement_ref); settlement-confirmed-late (confirmed_at, settlement_ref, delay_ms); settlement-failed (final_state); settlement-failed-orphaned (last_known_state)',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-005',
        keyword: 'MUST',
        summary: 'Unknown event_kind rejects with gateway.export.invalid_event_kind',
        source_fragment:
          'unknown event_kind rejects with gateway.export.invalid_event_kind; closed-enum discriminator over 8 *-observed event kinds (7 settlement/recovery states + 1 facilitator-timeout trigger observation)',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-006',
        keyword: 'MUST',
        summary:
          'Trigger-vs-state doctrine: 7 type URIs correspond to observed settlement/recovery states; gateway-facilitator-timeout-observed records the facilitator-timeout trigger event itself, not a settlement state; PEAC does NOT introduce a new settlement state',
        source_fragment:
          '7-state settlement/recovery model plus one PEAC timeout-trigger observation; gateway-facilitator-timeout-observed records the facilitator-timeout trigger event itself, not a settlement state; PEAC does not introduce a new settlement state',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-007',
        keyword: 'MUST',
        summary:
          'timeout_profile closed enum aligned with upstream environment profiles (datacenter / east_africa_3g / west_africa_3g / custom); caller-reported labels (PEAC does NOT infer geography, network quality, settlement finality, or settlement guarantees); custom REQUIRES facilitator_timeout_ms + poll_interval_ms + max_poll_window_ms (each non-negative safe integer, bounded 0..2_592_000_000); polling_strategy is a PEAC-defined OBSERVER-scope categorical field (not an upstream enum)',
        source_fragment:
          'timeout_profile closed enum (datacenter / east_africa_3g / west_africa_3g / custom) aligned with upstream environment profiles; caller-reported labels; PEAC does not infer geography, network quality, settlement finality, or settlement guarantees; custom requires facilitator_timeout_ms, poll_interval_ms, max_poll_window_ms (each non-negative safe integer bounded 0..2_592_000_000); polling_strategy is a PEAC observer categorization, not an upstream enum',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-008',
        keyword: 'MUST',
        summary:
          'Bounded numeric fields enforced; poll_count and check_count bounded to 0..1_000_000 reject with gateway.export.invalid_poll_count / gateway.export.invalid_check_count; deadline_exceeded_ms / delay_ms / facilitator_timeout_ms / poll_interval_ms / max_poll_window_ms bounded to 0..2_592_000_000 reject with corresponding stable codes; valid_before_unix_seconds is a non-negative safe integer (caller-reported EIP-3009 expiry in Unix seconds)',
        source_fragment:
          'poll_count and check_count bounded to 0..1_000_000; deadline_exceeded_ms / delay_ms / facilitator_timeout_ms / poll_interval_ms / max_poll_window_ms bounded to 0..2_592_000_000 (30 days); valid_before_unix_seconds non-negative safe integer; all numeric fields enforce Number.isSafeInteger',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-009',
        keyword: 'MUST',
        summary:
          'Every type URI in GATEWAY_EXPORT_TYPE_URIS maps to org.peacprotocol/gateway-export in TYPE_TO_EXTENSION_MAP; validateGatewayExportForType rejects unknown type URIs (type_uri_unknown) and type URI / event_kind mismatches (type_event_kind_mismatch)',
        source_fragment:
          'every URI in GATEWAY_EXPORT_TYPE_URIS maps to org.peacprotocol/gateway-export in TYPE_TO_EXTENSION_MAP; validateGatewayExportForType runtime guard: unknown type URI -> gateway.export.type_uri_unknown; mismatched event_kind -> gateway.export.type_event_kind_mismatch',
        enforcement_class: 'hard_fail',
      },
      {
        id: 'GATE-EXP-010',
        keyword: 'MUST',
        summary:
          'Spec boundary text is normative and vendor-neutral; PEAC does not settle transactions, route payments, contact gateways, verify on-chain state, monitor settlements, enforce recovery policy, resolve settlement disputes, or vouch for the legal validity of any settlement decision',
        source_fragment:
          'PEAC does not settle transactions, route payments, contact gateways, verify on-chain state, monitor settlements, enforce recovery policy, or resolve settlement disputes; recovery decisions are reported by the caller',
        enforcement_class: 'hard_fail',
      },
    ],
  },
