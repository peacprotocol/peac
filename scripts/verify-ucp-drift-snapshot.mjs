...
const REQUIRED_SOURCE_FIELDS = ['spec_url', 'repo', 'path', 'version_tag', 'raw_url'];
...
function latestVersionTag(repo) {
  const out = execFileSync(
    'git',
    ['ls-remote', '--tags', `https://github.com/${repo}.git`, 'refs/tags/v*'],
...
async function main() {
  ...
  // Lane 1: pinned snapshot integrity.
  try {
    const text = await fetchText(snapshot.source.raw_url);
    const missing = missingTokens(text, snapshot);
    if (missing.length) {
      failures.push(
      ...
      process.stdout.write(
        `Lane 1 OK: pinned ${snapshot.source.version_tag} still contains all ${invariantTokens(snapshot).length} invariants\n`
      );
    }
  } catch (err) {
  ...
  try {
    const latest = latestVersionTag(snapshot.source.repo);
    if (!latest) {
