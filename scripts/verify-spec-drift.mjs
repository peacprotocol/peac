...
function verifyDefaultPathTruth() {
  ...
  if (!hasIssues) console.log(colors.green('  PASS: No default-path legacy teaching found in quickstarts'));
  return hasIssues;
}
...
function verifyX402References() {
  ...
  // Advisory until x402 migration PR merges; becomes blocking after.
  const activeGlobs = [
  ...
  for (const glob of activeGlobs) {
    try {
      ...
      if (output.trim()) {
        console.log(colors.yellow(`  WARN: coinbase/x402 found in active scope (migrate to x402-foundation/x402):`));
  ...
  // Advisory, not blocking: the x402 migration is a separate PR
  return false;
}
