...
export class Http402Handler {
  ...
  async createResponse(ctx: NegotiationContext, instance?: string): Promise<Http402Response> {
    ...
    headers['X-Payment-Amount'] = `${primaryChallenge.amount.value} ${primaryChallenge.amount.currency}`;
  ...
  parsePaymentHeader(header: string): { rail: string; evidence: string } | null {
    ...
    const parts = direct.trim().split(/\s+/).filter(p => p);
  ...
  async verifyPayment(paymentHeader: string, originalChallenge?: string): Promise<import('./types.js').PaymentEvidence | null> {
...
export async function create402Response(
  ...
  const { X402MockAdapter, TempoMockAdapter, L402MockAdapter, StripeMockAdapter } = await import('./negotiator.js');
