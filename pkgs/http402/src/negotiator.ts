...
export class PaymentNegotiator {
  ...
  parseAcceptPayments(header?: string, isDev = false): PaymentRail[] {
    ...
    for (const token of tokens) {
      ...
      const q = qPart?.includes('q=') 
        ? parseFloat(qPart.split('q=')[1]) || 1.0 
        : 1.0;
  ...
  async verify(rail: PaymentRail, challenge: string, evidence: string): Promise<import('./types.js').PaymentEvidence | null> {
    const adapter = this.adapters.get(rail);
    if (!adapter) {
...
export class X402MockAdapter implements PaymentAdapter {
  ...
  async verify(challenge: string, evidence: string): Promise<import('./types.js').PaymentEvidence | null> {
...
export class TempoMockAdapter implements PaymentAdapter {
  ...
  async verify(challenge: string, evidence: string): Promise<import('./types.js').PaymentEvidence | null> {
    ...
    const providerIds = evidenceParts.filter(part => 
